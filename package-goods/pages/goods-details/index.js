...
Page({
  ...
  goShopCar: function () {
    wx.reLaunch({
      url: "/package-shop/pages/shop-cart/index"
    });
  },
  async toAddShopCar() {
    this.bindGuiGeTap();
  ...
  onShareAppMessage() {
    let _data = {
      ...
      // path: '/package-goods/pages/goods-details/index?id=' + this.data.goodsDetail.basicInfo.id + '&inviter_id=' + wx.getStorageSync('uid'),
      path: '/package-goods/pages/goods-details/index?id=' + this.data.goodsDetail.basicInfo.id + '&inviter_id=' + getApp().globalData.uid,
  ...
  async drawSharePic() {
    ...
    const qrcodeRes = await WXAPI.wxaQrcode({
      ...
      page: 'package-goods/goods-details/index',
      is_hyaline: true,
      autoColor: true,
      expireHours: 1
    })
